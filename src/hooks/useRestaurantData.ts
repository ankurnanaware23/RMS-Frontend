...
export function useRestaurantData() {
    ...
    try {
      ...
      const ordersWithCategory = ordersData.map((order) => ({
        ...
        items: order.items.map((item) => ({
          ...item,
          category: item.dishId
            ? dishCategoryMap.get(item.dishId) || item.category
            : item.category,
        })),
      }));
  ...
  const addTable = useCallback(
    async (tableData: Omit<Table, "id" | "createdAt" | "updatedAt">) => {
      try {
  ...
  const updateTable = useCallback(
    async (
      ...
      try {
        ...
        toast({
          title: "Table Updated",
          description: `Table ${updatedTable.number} updated.`,
        });
      } catch (error) {
        toast({
          title: "Error",
  ...
  const bookTable = useCallback(
    async (
      ...
      try {
        const updatedTable = await api.bookTable(
          tableId,
          customerName,
          reservationDateTime
        );
        ...
        toast({
          title: "Table Booked",
          description: `Table ${updatedTable.number} has been booked.`,
        });
        fetchData();
      } catch (error) {
        toast({
          title: "Error",
          description: "Failed to book table.",
          variant: "destructive",
        });
      }
    },
  ...
  const addOrder = useCallback(
    async (orderData: Omit<Order, "id" | "createdAt" | "updatedAt">) => {
      try {
          ...
          if (existingOrder) {
            ...
            orderData.items.forEach((item) => {
              ...
              const existingItem = mergedItems.find(
                (existing) => (existing.dishId || existing.id) === dishId
              );
              if (existingItem) {
            ...
            toast({
              title: "Order Updated",
              description: `Order for Table ${existingOrder.tableNumber} updated.`,
            });
            fetchData();
            return;
          }
        ...
        toast({
          title: "Order Added",
          description: `Order for ${newOrder.customerName} has been added.`,
        });
        fetchData();
      } catch (error) {
        toast({
  ...
  const updateOrderStatus = useCallback(
    async (orderId: string, status: Order["status"]) => {
      try {
        ...
        setOrders((prev) =>
          prev.map((orderItem) =>
            orderItem.id === orderId ? { ...orderItem, status } : orderItem
          )
        );
  ...
  const completeOrderPayment = useCallback(
    async (orderId: string) => {
      try {
        ...
        toast({
          title: "Payment Done",
          description: "Order completed and table freed.",
        });
        fetchData();
      } catch (error) {
        toast({
  ...
  const updateOrderItems = useCallback(
    async (orderId: string, items: { dishId: string; quantity: number }[]) => {
      try {
